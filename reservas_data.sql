...
BEGIN
    ...
    -- Verificar que existen usuarios (convertir id a texto para comparación)
    IF NOT EXISTS (SELECT 1 FROM users WHERE id::text ~ '^[0-9]+$' AND id::text::integer BETWEEN 5 AND 67) THEN
    ...
    FOR day_counter IN 0..6 LOOP
        ...
        IF NOT is_weekend THEN
            ...
            FOR counter IN 1..reservas_turno1 LOOP
                ...
                FROM users 
                WHERE id::text ~ '^[0-9]+$' AND id::text::integer BETWEEN 5 AND 67 
                ORDER BY RANDOM() 
                LIMIT 1;
...
ORDER BY 
    CASE franja_horaria
        WHEN '17:00-17:30' THEN 1
        WHEN '18:15-18:35' THEN 2  
        WHEN '19:00-19:30' THEN 3
        WHEN '19:30-19:55' THEN 4
        ELSE 5
    END;
...
SELECT 
    ...
    TO_CHAR(created_at::date, 'Day') as dia_semana,
    COUNT(*) as total_reservas_dia,
    COUNT(*) FILTER (WHERE franja_horaria = '17:00-17:30') as turno_1,
    COUNT(*) FILTER (WHERE franja_horaria = '18:15-18:35') as turno_2,
    COUNT(*) FILTER (WHERE franja_horaria = '19:00-19:30') as turno_3,
    COUNT(*) FILTER (WHERE franja_horaria = '19:30-19:55') as turno_4,
    ROUND(AVG(CASE WHEN estado = 'validado' THEN 1.0 ELSE 0.0 END) * 100, 1) as porcentaje_validacion
FROM reservas
WHERE created_at >= CURRENT_DATE - INTERVAL '8 days'
...
SELECT 
    ...
    u.facultad,
    COUNT(r.*) as total_reservas,
    COUNT(*) FILTER (WHERE r.tipo_pase = 'asiento') as reservas_asiento,
    COUNT(*) FILTER (WHERE r.tipo_pase = 'parado') as reservas_parado,
    COUNT(*) FILTER (WHERE r.estado = 'validado') as reservas_validadas
FROM reservas r
JOIN users u ON r.id_usuario = u.id
WHERE r.created_at >= CURRENT_DATE - INTERVAL '8 days'
GROUP BY u.id, u.nombres, u.apellidos, u.facultad
ORDER BY total_reservas DESC
LIMIT 10;

-- 5. Análisis de ubicaciones GPS
SELECT 
    'Análisis de coordenadas GPS' as titulo,
    COUNT(*) as reservas_con_gps,
    ROUND(AVG(ubicacion_lat), 6) as latitud_promedio,
    ROUND(AVG(ubicacion_lng), 6) as longitud_promedio,
    ROUND(MIN(ubicacion_lat), 6) as lat_min,
    ROUND(MAX(ubicacion_lat), 6) as lat_max,
    ROUND(MIN(ubicacion_lng), 6) as lng_min,
    ROUND(MAX(ubicacion_lng), 6) as lng_max
FROM reservas
WHERE created_at >= CURRENT_DATE - INTERVAL '8 days'
    AND ubicacion_lat IS NOT NULL 
    AND ubicacion_lng IS NOT NULL;

-- 6. Función get_reservation_counts() - Ejemplo de uso para HOY
SELECT 
    '📊 CONTEOS HOY (usando función RPC)' as titulo,
    franja_horaria,
    total_reservas,
    asientos_ocupados,
    parados_ocupados,
    (asientos_ocupados + parados_ocupados) as ocupacion_total
FROM get_reservation_counts();
